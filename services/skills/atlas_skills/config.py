...
"""Atlas Skill Manager configuration (12-factor, validated at boot)."""
...
class SkillsConfig(BaseSettings):
    model_config = SettingsConfigDict(env_prefix="ATLAS_SKILLS_", env_file=".env", extra="ignore")

    # Network
    host: str = "0.0.0.0"
    port: int = Field(default=8800, ge=1, le=65535)
    self_url: str = "https://atlas-memory:8800"
    ...
    bootstrap_token: str = Field(
        ...
        validation_alias=AliasChoices(
            "ATLAS_SKILLS_BOOTSTRAP_TOKEN", "ATLAS_BOOTSTRAP_TOKEN", "bootstrap_token"
        ),
    )
    ...
    security_mode: str = Field(
        default="mtls",
        validation_alias=AliasChoices(
            "ATLAS_SKILLS_SECURITY_MODE", "ATLAS_SECURITY_MODE", "security_mode"
        ),
    )
    tls_dir: str = "data/tls"
    ca_cert_file: str | None = Field(
        default=None,
        validation_alias=AliasChoices("ATLAS_SKILLS_CA_CERT", "ATLAS_CA_CERT", "ca_cert_file"),
    )
    introspect_cache_ttl_seconds: float = Field(default=30.0, ge=0)

    # Storage
    database_path: str = "data/atlas-skills.db"
