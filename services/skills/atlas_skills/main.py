...
"""Atlas Skill Manager application factory and lifecycle."""
...
log = logging.getLogger("atlas.skills")
...
def build_atlas_service(config: SkillsConfig) -> AtlasService:
    return AtlasService(
        name=SERVICE_NAME,
        version=__version__,
        address=config.self_url,
        health_url=f"{config.self_url}/healthz",
        capabilities=["skills.publish", "skills.discover"],
        metadata={"description": "Atlas Skill Manager — signed, versioned capability packages"},
...
async def _lifespan(app: FastAPI):
    config: SkillsConfig = app.state.config

    store = SkillStore(config.database_path)
    await store.open()
    app.state.store = store
    log.info("skill store initialized (db=%s)", config.database_path)
    ...
    # Trust anchor for skill signatures: the Atlas CA certificate. In mtls
    # mode the SDK already fetched it at enrollment; token mode reads the
    # configured file, or skills cannot be verified.
    ca_pem = None
    if atlas.tls is not None and atlas.tls.ca_path.exists():
        ca_pem = atlas.tls.ca_path.read_bytes()
    elif config.ca_cert_file:
        from pathlib import Path as _P
        ca_pem = _P(config.ca_cert_file).read_bytes()
    app.state.ca_cert_pem = ca_pem
    if ca_pem is None:
        log.warning("no CA certificate available — skill publication will be refused")

    log.info("Atlas Skill Manager ready (%s v%s)", SERVICE_NAME, __version__)
    try:
        yield
    finally:
        log.info("Atlas Skill Manager shutting down")
        await outbox.stop()
        await atlas.stop()
...
def create_app(
    config: SkillsConfig | None = None, atlas: AtlasService | None = None
) -> FastAPI:
    if config is None:
        config = SkillsConfig()
    _configure_logging(config.log_level)
    app = FastAPI(
        title="Atlas Skill Manager",
        version=__version__,
        description="Signed, versioned, discoverable capability packages.",
        lifespan=_lifespan,
    )
...
def run() -> None:
    ...
    try:
        ...
        print(f"FATAL: Atlas Skill Manager refused to boot: {exc}", file=sys.stderr)
