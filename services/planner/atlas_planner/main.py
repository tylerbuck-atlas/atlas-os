...
"""Atlas Planner application factory and lifecycle."""
...
def build_atlas_service(config: PlannerConfig) -> AtlasService:
    return AtlasService(
        name=SERVICE_NAME,
        version=__version__,
        address=config.self_url,
        health_url=f"{config.self_url}/healthz",
        capabilities=["planner.plans.submit", "planner.policies"],
        metadata={"description": "Atlas Planner — the only path from intent to action"},
...
async def _lifespan(app: FastAPI):
    config: PlannerConfig = app.state.config

    store = PlannerStore(config.database_path)
    await store.open()
    app.state.store = store
    log.info("planner store initialized (db=%s)", config.database_path)
    ...
    engine = PlannerEngine(store, atlas, invoke_timeout=config.invoke_timeout_seconds)
    app.state.engine = engine

    log.info("Atlas Planner ready (%s v%s) — default deny; policies decide", SERVICE_NAME, __version__)
    try:
        yield
    finally:
        log.info("Atlas Planner shutting down")
        await engine.close()
        await outbox.stop()
        await atlas.stop()
...
def create_app(
    config: PlannerConfig | None = None, atlas: AtlasService | None = None
) -> FastAPI:
    if config is None:
    ...
    app = FastAPI(
        title="Atlas Planner",
        version=__version__,
        description="Goal -> validated plan -> auditable execution. Default deny.",
        lifespan=_lifespan,
    )
...
def run() -> None:
    ...
    try:
        ...
        print(f"FATAL: Atlas Planner refused to boot: {exc}", file=sys.stderr)
