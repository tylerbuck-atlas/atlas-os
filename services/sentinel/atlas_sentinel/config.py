...
"""Atlas Sentinel configuration (12-factor, validated at boot)."""
...
class SentinelConfig(BaseSettings):
    model_config = SettingsConfigDict(env_prefix="ATLAS_SENTINEL_", env_file=".env", extra="ignore")

    # Network
    host: str = "0.0.0.0"
    port: int = Field(default=8600, ge=1, le=65535)
    self_url: str = "https://atlas-memory:8600"
    ...
    bootstrap_token: str = Field(
        ...
        validation_alias=AliasChoices(
            "ATLAS_SENTINEL_BOOTSTRAP_TOKEN", "ATLAS_BOOTSTRAP_TOKEN", "bootstrap_token"
        ),
    )
    ...
    security_mode: str = Field(
        default="mtls",
        validation_alias=AliasChoices(
            "ATLAS_SENTINEL_SECURITY_MODE", "ATLAS_SECURITY_MODE", "security_mode"
        ),
    )
    tls_dir: str = "data/tls"
    ca_cert_file: str | None = Field(
        default=None,
        validation_alias=AliasChoices("ATLAS_SENTINEL_CA_CERT", "ATLAS_CA_CERT", "ca_cert_file"),
    )
    introspect_cache_ttl_seconds: float = Field(default=30.0, ge=0)

    # Anomaly rule thresholds
    flap_threshold: int = Field(default=4, ge=2)
    flap_window_seconds: float = Field(default=60.0, gt=0)
    rejection_threshold: int = Field(default=3, ge=2)
    rejection_window_seconds: float = Field(default=300.0, gt=0)
    alert_cooldown_seconds: float = Field(default=60.0, ge=0)

    # Storage
    database_path: str = "data/atlas-sentinel.db"
