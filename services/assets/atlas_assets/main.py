...
"""Atlas Asset Manager application factory and lifecycle."""
...
log = logging.getLogger("atlas.assets")
...
def build_atlas_service(config: AssetsConfig) -> AtlasService:
    return AtlasService(
        name=SERVICE_NAME,
        version=__version__,
        address=config.self_url,
        health_url=f"{config.self_url}/healthz",
        capabilities=["assets.ingest", "assets.read", "assets.content"],
        metadata={"description": "Atlas Asset Manager — files, manuals, documents"},
...
async def _lifespan(app: FastAPI):
    config: AssetsConfig = app.state.config

    store = AssetStore(config.database_path, config.blob_dir)
    await store.open()
    app.state.store = store
    log.info("asset store initialized (db=%s, blobs=%s)", config.database_path, config.blob_dir)
    ...
    log.info("Atlas Asset Manager ready (%s v%s)", SERVICE_NAME, __version__)
    try:
        yield
    finally:
        log.info("Atlas Asset Manager shutting down")
        await outbox.stop()
        await atlas.stop()
...
def create_app(
    config: AssetsConfig | None = None, atlas: AtlasService | None = None
) -> FastAPI:
    if config is None:
        config = AssetsConfig()
    _configure_logging(config.log_level)
    app = FastAPI(
        title="Atlas Asset Manager",
        version=__version__,
        description="The home's files, manuals, and documents as auditable truth sources.",
        lifespan=_lifespan,
    )
...
def run() -> None:
    ...
    try:
        ...
        print(f"FATAL: Atlas Asset Manager refused to boot: {exc}", file=sys.stderr)
